...
cat > /etc/systemd/system/$KEY_LOWER@.service <<-EOF
[Unit]
Description=$KEY_LOWER Service
Documentation=http://halucok.me
Documentation=https://t.me/sampiiiiu
After=network.target nss-lookup.target
...
AmbientCapabilities=CAP_NET_ADMIN CAP_NET_BIND_SERVICE
NoNewPrivileges=true
ExecStart=/usr/bin/$KEY_LOWER/$KEY_LOWER -config /etc/$KEY_LOWER/%i.json
Restart=on-failure
...
Help(){
   cat - 1>& 2 << EOF ./install-release.sh [-h] [-c] [--remove] [-p proxy] [-f] [--version vx.y.z] [-l file]
   -h, --help            Show help
